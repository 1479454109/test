...
const selectAddress = computed(() => {
  ...
  return (
    store.selectedAddress || orderPre.value?.userAddresses.find((x: AddressItem) => x.isDefault)
  )
})
...
const getOrderPreNow = async () => {
  const res = await getMemberOrderPreNowAPI({ skuId: prop.skuId, count: prop.count + '' })
  orderPre.value = res.result
}
