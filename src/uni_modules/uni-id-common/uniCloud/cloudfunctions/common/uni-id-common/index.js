...
/* eslint-disable */
"use strict";
var e, t = (e = require("crypto")) && "object" == typeof e && "default" in e ? e.default : e;
const n = {
...
function a(e) {
	return "object" === (i = e, Object.prototype.toString.call(i).slice(8, -1).toLowerCase()) && e.errCode && (t = e
		.errCode, Object.values(n).includes(t)) && !!e.errCode;
	var t, i
}
let u = {
	"zh-Hans": {
...
class m {
	constructor({
		...
		this.uid = null, this.userRecord = null, this.userPermission = null, this.oldToken = null, this
			.oldTokenPayload = null, this.uniId = e, this.config = this.uniId._getConfig(), this.clientInfo = this
			.uniId._clientInfo, this.checkConfig()
	}
	checkConfig() {
		const {
			tokenExpiresIn: e,
			tokenExpiresThreshold: t
		} = this.config;
	...
	async getUserRecord() {
		...
		if (this.userRecord = e.data[0], !this.userRecord) throw {
			errCode: n.ACCOUNT_NOT_EXISTS
		};
		switch (this.userRecord.status) {
			...
			case 1:
				throw {
					errCode: n.ACCOUNT_BANNED
				};
			case 2:
				throw {
					errCode: n.ACCOUNT_AUDITING
				};
			case 3:
				throw {
					errCode: n.ACCOUNT_AUDIT_FAILED
				};
			case 4:
				throw {
					errCode: n.ACCOUNT_CLOSED
				}
		}
		if (this.oldTokenPayload) {
			if (this.isTokenInDb(this.oldTokenPayload.uniIdVersion)) {
				if (-1 === (this.userRecord.token || []).indexOf(this.oldToken)) throw {
					errCode: n.CHECK_TOKEN_FAILED
				}
			}
			if (this.userRecord.valid_token_date && this.userRecord.valid_token_date > 1e3 * this.oldTokenPayload
				.iat) throw {
				errCode: n.TOKEN_EXPIRED
			}
		}
	...
	async getUserPermission() {
		...
		if (0 === e.length) return this.userPermission = {
			role: [],
			permission: []
		}, this.userPermission;
		if (e.includes("admin")) return this.userPermission = {
			role: e,
			permission: []
		}, this.userPermission;
		const t = await T.where({
				role_id: _.in(e)
			}).get(),
			n = (i = t.data.reduce((e, t) => (t.permission && e.push(...t.permission), e), []), Array.from(new Set(
				i)));
		var i;
	...
	async _createToken({
		...
		return l.push(u), l.length > a && l.splice(0, l.length - a), await this.updateUserRecord({
			last_login_ip: this.clientInfo.clientIP,
			last_login_date: o,
			token: l
		}), {
			token: u,
			tokenExpired: o + 1e3 * c
		}
	}
	async createToken({
		uid: e,
	...
	_checkToken(e) {
		...
		try {
			...
			if ("TokenExpiredError" === e.name) throw {
				errCode: n.TOKEN_EXPIRED
			};
			throw {
				errCode: n.CHECK_TOKEN_FAILED
	...
	async checkToken(e, {
		...
		const u = Date.now();
		let d = {};
		1e3 * i.exp - u < 1e3 * r && (d = await this._createToken({
			uid: o
		}));
		const l = {
			code: 0,
...
const w = require("uni-config-center")({
	pluginId: "uni-id"
});
class x {
	constructor({
		...
		}(e) : t, this.config = n || this._getOriginConfig(), this.interceptorMap = new Map, w.hasFile(
			"custom-token.js") && this.setInterceptor("customToken", require(w.resolve("custom-token.js")));
	...
	_getAppConfig() {
		...
		return Array.isArray(e) ? e.find(e => e.dcloudAppid === this._clientInfo.appId) || e.find(e => e
			.isDefaultConfig) : e
	}
	_getPlatformConfig() {
		...
		let t;
		switch ("app-plus" === this._clientInfo.platform && (this._clientInfo.platform = "app"), "h5" === this
			._clientInfo.platform && (this._clientInfo.platform = "web"), this._clientInfo.platform) {
			case "web":
				t = "h5";
				break;
			case "app":
		...
		return ["tokenSecret", "tokenExpiresIn"].forEach(e => {
		...
		get(e, t) {
			if (t in e && 0 !== t.indexOf("_")) {
				if ("function" == typeof e[t]) return (n = e[t], function() {
					...
					return i(e) ? e.then(e => (a(e) && c.call(this, e), e), e => {
						if (a(e)) return c.call(this, e), e;
						throw e
					}) : (a(e) && c.call(this, e), e)
				}).bind(e);
