...
const isSelectedAll = computed(() => {
  return cartList.value.length && cartList.value.every((v: any) => v.selected)
})
...
const onChangeSelectedAll = () => {
  ...
  // 前端数据更新
  cartList.value.forEach((item: CartItem) => {
...
const selectedCartListCount = computed(() => {
  return selectedCartList.value.reduce((sum: any, item: CartItem) => sum + item.count, 0)
})

// 计算选中总金额
const selectedCartListMoney = computed(() => {
  return selectedCartList.value
    .reduce((sum: any, item: CartItem) => sum + item.count * item.nowPrice, 0)
    .toFixed(2)
})
